...
for i in range(128):
    if i > 0:
        ...
        p("vpand %ymm{}, %ymm{}, %ymm{}".format(0, 1, 2))
    for j in range(256):
        if i == 0:
            p("vpand {}(%rdx), %ymm0, %ymm3".format((i * 256 + j) * 32))  # AND with F.
        else:
            p("vpand {}(%rdx), %ymm2, %ymm3".format((i * 256 + j) * 32))  # AND with F.
...
p("vpand %ymm0, %ymm1, %ymm1")

for i in range(256):
    p("vpand {}(%rdx), %xmm1, %xmm0".format((128 * 256 * 32) + i * 16))  # AND with F.
