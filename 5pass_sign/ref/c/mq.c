                             ...
                             const gf31 *yi, const gf31 *yj)
{
    ...
    for (i = 0; i < 16; i += 2) {
        for (j = 0; j < 16; j++) {
            x1[j] = (xi[i]*yj[j] + yi[i]*xj[j]) % 31;
        }
        for (j = 0; j < 16; j++) {
            x2[j] = (xi[i + 1]*yj[j] + yi[i + 1]*xj[j]) % 31;
        }
...
void generate_xiyj_p_xjyi_tri_n32(gf31 *xij, const gf31 *x, const gf31 *y)
{
    ...
    for (i = 0; i < 8;i++) {
        ...
        ror_16x2(y_rot, y, i);
        for (j = 0; j < 16; j++) {
            x1[j] = (x[j] * y_rot[j] + y[j] * x_rot[j]) % 31;
        }
        for (j = 0; j < 16; j++) {
            x2[j] = (x[j + 16]*y_rot[j + 16] + y[j + 16]*x_rot[j + 16]) % 31;
        }
        pack_store(xij + k, x1, x2);
        k += 32;
    }
